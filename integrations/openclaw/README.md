...
```text
Install agentmemory for OpenClaw. Run `bunx --bun @agentmemory/agentmemory` in a separate terminal to start the memory server on localhost:3111.
...
**Connection refused on port 3111** — the agentmemory server is not running. Start it with `bunx --bun @agentmemory/agentmemory`.
