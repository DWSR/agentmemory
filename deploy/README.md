...
`@agentmemory/agentmemory` from npm at build time with Bun and copies the iii
...
The entrypoint overwrites the Bun-bundled iii config with a
deploy-tuned one that binds `0.0.0.0` and uses absolute `/data` paths,
then drops privileges from `root` to `bun` via `gosu` before
