...
agentmemory는 iii-engine v0.11.6를 고정하며 다른 버전에는 연결되지 않습니다(워커가 다른 엔진의 프로토콜을 말할 수 없습니다). 다른 엔진을 중지한 후 `bunx --bun @agentmemory/agentmemory@latest`를 실행하십시오. 고정된 v0.11.6를 `~/.agentmemory/bin`에 설치·실행하며, 기존 `iii`는 건드리지 않습니다.
...
호환성: 이 릴리스는 안정 버전 `iii-sdk` `^0.11.0`과 iii-engine v0.11.6를 대상으로 합니다.
...
경고: 이 명령은 현재 workspace/런타임을 변경합니다. JavaScript 의존성을 업데이트할 수 있으며, 고정된 Docker 이미지 `iiidev/iii:0.11.6`를 pull할 수 있습니다. 고정되지 않았거나 더 새로운 iii 엔진을 설치하는 일은 절대 없습니다.
...
`iii-engine`을 수동으로 설치하십시오. **agentmemory는 `iii-engine`과 `iii-sdk`를 일치하는 `0.11.6` 버전으로 고정합니다**. Worker는 SDK를 통해 직접 등록되므로 `iii worker add` 마이그레이션이 필요하지 않습니다. 의도적으로 다른 엔진을 관리할 때만 `AGENTMEMORY_III_VERSION=<version>`으로 덮어쓰십시오.
...
- **Windows:** [iii-hq/iii releases v0.11.6](https://github.com/iii-hq/iii/releases/tag/iii%2Fv0.11.6)에서 `iii-x86_64-pc-windows-msvc.zip`을 다운로드하고 `iii.exe`를 추출한 후 PATH에 추가

또는 Docker 사용 (번들된 `docker-compose.yml`이 `iiidev/iii:0.11.6`를 pull). 전체 문서: [iii.dev/docs](https://iii.dev/docs).
...
> 참고: iii **엔진**은 사전 빌드된 바이너리이며 cargo 크레이트가 아니므로, `cargo install`로 설치하려 하지 마세요. (iii **SDK**는 crates.io, npm, PyPI에 게시되어 있지만 agentmemory에는 필요하지 않습니다.) 지원되는 엔진 설치 방법은 모두 v0.11.6에 고정되어 있습니다: 위의 사전 빌드된 v0.11.6 바이너리, 버전 핀**을 포함한** 업스트림 `sh` 설치 스크립트 `curl -fsSL https://install.iii.dev/iii/main/install.sh | VERSION=0.11.6 sh` (macOS/Linux), 그리고 Docker 이미지 `iiidev/iii:0.11.6`. 그냥 `install.sh | sh`를 실행하면 **최신** 엔진이 설치되는데, agentmemory는 이를 지원하지 않습니다; 항상 `VERSION=0.11.6`을 전달하세요. 가장 쉬운 방법은 그냥 `bunx --bun @agentmemory/agentmemory`를 실행하는 것입니다. 이 명령이 고정된 엔진을 `~/.agentmemory/bin`에 가져다 줍니다.
