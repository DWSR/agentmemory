...
agentmemory is a local memory server for coding agents. It exposes a REST API and MCP HTTP routes on port 3111, streams on 3112, a viewer on 3113, and connects its worker to iii-engine on 49134. It is built on the iii engine, which agentmemory manages as a pinned v0.11.6 binary under `~/.agentmemory/bin` on macOS/Linux or `%USERPROFILE%\.agentmemory\bin` on Windows.
...
- Windows: WSL2 follows the Linux path above. Native Windows requires a manually downloaded, pinned iii-engine v0.11.6 `iii.exe`, or Docker Desktop; the CLI does not auto-extract the Windows ZIP. Native automated `connect` supports only `copilot-cli`. Other Windows agents need manual MCP configuration; WSL `connect` applies only to agents installed in that same WSL environment.
...
# https://github.com/iii-hq/iii/releases/tag/iii%2Fv0.11.6
# Extract iii.exe to $HOME\.agentmemory\bin\iii.exe, then verify the pin:
& "$HOME\.agentmemory\bin\iii.exe" --version
```

Expect: exactly `0.11.6`. Do not install the latest unpinned iii release. Alternatively, use WSL2 or start Docker Desktop and choose the Docker path when agentmemory starts.
...
Expect: iii-engine v0.11.6 starts, the agentmemory worker registers with iii, and the ready panel lists REST, viewer, streams, and engine addresses. First boot can take longer while the engine binary is downloaded.
...
- Native Windows: the CLI does not auto-extract the ZIP. Install the pinned v0.11.6 `iii.exe` manually, use WSL2, or run with Docker Desktop and `AGENTMEMORY_USE_DOCKER=1`. Automated `connect` supports only `copilot-cli`; configure other native agents manually.
...
- iii-engine v0.11.6 running, the selected platform data directory, and all four ports accounted for
