...
if ! command -v iii >/dev/null 2>&1; then
  ...
  echo "  curl -fsSL https://github.com/iii-hq/iii/releases/download/iii/v0.20.0/iii-aarch64-apple-darwin.tar.gz | tar -xz -C ~/.local/bin"
  exit 1
fi

iii_ver=$(iii --version 2>&1 | head -1)
if [[ "$iii_ver" != "0.20.0" ]]; then
  echo "warning: iii version on PATH is $iii_ver; agentmemory pins 0.20.0"
fi
