...
const IIPINNED_VERSION =
  process.env["AGENTMEMORY_III_VERSION"] || "0.20.0";
...
function iiiReleaseUrl(): string | null {
  ...
  // Tag name is monorepo-prefixed: `iii/v0.20.0`. Slash is URL-encoded
...
async function runUpgrade() {
  ...
  if (hasPackageJson) {
    if (bunBin) {
      ...
      runCommand(bunBin, ["add", "--exact", "iii-sdk@0.20.0"], {
        label: "Pinning iii-sdk@0.20.0",
        optional: true,
      });
    } else {
