...
const IIPINNED_VERSION =
  process.env["AGENTMEMORY_III_VERSION"] || "0.16.1";
...
function iiiReleaseUrl(): string | null {
  ...
  // Tag name is monorepo-prefixed: `iii/v0.16.1`. Slash is URL-encoded
...
async function runUpgrade() {
  ...
  if (hasPackageJson) {
    if (bunBin) {
      ...
      runCommand(bunBin, ["add", "--exact", "iii-sdk@0.16.1"], {
        label: "Pinning iii-sdk@0.16.1",
        optional: true,
      });
    } else {
